...
- 오답 is shown in 멘토링 by `wrongListHtml(exam, sc)` — one section per subject with an answer key and saved answers, chips reading `문항번호 [선택] 학생답 → 정답`, 미입력 rendered grey. It appears in both `renderConsultDetail` (컨설팅 탭) and `renderTabletConsult` (멘토링 태블릿) under the 성적표 card.
