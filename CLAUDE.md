...
- **한 스레드에 문제는 2개까지.** 상한은 서버 `tokens.js` 의 `MAX_PROBLEMS` 와 화면의 `QA_MAX_PROBLEMS` **두 군데에 같은 값**으로 있다(질문 작성은 `tokenApi` 를 타므로 화면 상수만 고치면 서버가 잘라 버린다). 학생이 작성 화면에서 [문제 1개]/[문제 2개] 를 고르고, `createQuestion` 이 `normProblems` 로 1~상한 사이로 자른 뒤(없거나 이상한 값은 1) 그 배수로 잔액을 확인·차감하고 `problems`·`tokenCost` 를 문서에 남긴다. **개수를 글에서 세지는 않는다** — 셀 방법이 없고 잘못 막으면 정상 질문이 거부되므로, 3개 이상을 적는 것은 예전처럼 안내로만 말린다. 문구는 작성 화면·스레드 상세·[토큰·결제] 안내 세 군데에 같이 둔다(한 군데만 적으면 이미 스레드를 연 학생은 못 본다).
  ...
  - **댓글도 문제마다 따로 단다.** 댓글의 `pi`(0부터)가 어느 문제에 달린 것인지 말한다. 칸이 **없거나 범위를 벗어나면 첫 문제**로 본다(`normProblemIndex`, 화면의 `qaCmtPi` 도 같은 판정) — 칸 없이 오는 경로가 둘 있는데(기능 이전에 달린 댓글, `index.html` 을 번들로 들고 있는 스토어 배포본) 거부할 수는 없고(`problems` 가 없을 때 1개로 보는 것과 같은 이유), **어디에도 붙지 않는 댓글을 두면 화면에 그것만 모으는 칸이 따로 생긴다**. 문제가 하나면 `pi` 를 아예 두지 않는다.
  - **문제마다 관리자 답이 있어야 답변완료다**(`allAnswered`). 하나만 답하고 답변완료로 넘어가면 남은 문제가 답변대기 필터에서 사라지는 데다, **자동 종료 시계까지 돌기 시작해 답을 못 받은 채 닫힌다**. 댓글을 지워 답이 빈 문제가 생기면 답변대기로 되돌린다 — `deleteQuestionComment` 도 같은 판정을 쓴다. 칸마다 `답변대기`/`답변완료` 칩이 따로 붙어 어느 문제가 남았는지 목록을 열지 않고 안다.
  - **나란히가 기본이고, 자세히 보기는 한 칸을 넓히는 것뿐이다.** 두 문제가 나란히 놓이고(`.qa-pcols` / `.qa-pb`) 칸마다 그 문제의 댓글과 입력칸이 따로 붙는다. 칸의 **머리줄과 본문**을 누르면 그 칸만 폭을 다 쓰고(`qaTapProblem` → `S.qa.focusPi`), 머리줄 오른쪽의 `← 돌아가기`(`qaUnfocus`)로 원래대로 돌아온다 — 별도의 보기 모드가 아니라 **열고 닫는 한 자리**다(`자세히 보기 →` 가 있던 그 자리가 나가는 자리가 된다). 사진은 누름 자리에서 뺀다 — 크게 보는 라이트박스가 이미 붙어 있다.
  - **`S.qa.focusPi` 는 `Number()` 로 받으면 안 된다** — `Number(null)` 이 `0` 이라 확대하지 않은 상태가 0번 문제 확대가 된다.
  - **입력칸 id 와 사진 스코프는 문제마다 갈린다**(`qaCmtTaId` / `qaCmtScope`) — 한 화면에 입력칸이 둘 떠 있어 같은 id 를 쓰면 서로 덮는다. 문제가 하나면 예전 값(`qa-cmt-<id>` / `qa_c_<id>`)을 그대로 써서 붙여 둔 초안이 어긋나지 않는다.
  - 토큰·새 질문·자동 종료 안내는 `qaThreadNotesHtml` 이 스레드 아래에 **한 번만** 둔다. 칸마다 되풀이하면 같은 말이 두 번 보인다. 머리줄도 열렸을 때와 닫혔을 때가 같은 모양이라, 넓어진 것 말고는 달라지는 게 없다.
...
**`fmIco()` 는 크기·선 색을 SVG 의 *속성*으로 박는다**(`width`/`height`/`stroke="currentColor"`/`fill="none"`). 속성은 CSS 보다 우선순위가 낮아 `.btn svg`·`.sn-ico` 같은 기존 규칙이 그대로 이기고, **그런 규칙이 없는 자리에서도 아이콘이 보인다**. 처음에는 부모 CSS 에만 기댔다가 규칙이 없는 자리(예약 잠금 표시·달력 배지·칩 안)에서 아무것도 안 보였다. 새 자리에 아이콘을 넣을 때 부모 CSS 를 따로 쓸 필요가 없다.
...
같은 화면에 표·칩·버튼이 잔뜩 들어가는 곳이라 학생 화면과 같은 크기로 두면 빽빽해 보인다. **`#vAdmin` 안에서만** 줄인다(표 12.5/12, 표머리 11.5, 섹션 제목 14.5/14, 칩 10.5 …) — 학생 화면은 그대로다. 손대려면 그 한 블록만 보면 된다. **누르는 자리는 글자만 줄이고 높이는 그대로 둔다** — 작아지면 손가락이 빗나간다.

### 학생 목록은 한 명이 한 줄

좌석·학년·학교를 따로 칸으로 두지 않고 **이름 옆에 붙인다**(`.stu-line`). 칸이 일곱이던 표가 `[체크] [학생] [관리]` 셋으로 줄었다. **휴대폰의 카드 모드에서도 한 줄이다** — `.tw-slim` 이 `tr` 을 `display:flex` 로 두어 이름 줄과 버튼을 나란히 놓는다. 위아래로 나누면 `cell-head` 의 아래 선과 `cell-actions` 의 위 선 사이가 텅 비어 카드만 커진다. 폭이 모자라면 **학교 이름부터 말줄임**으로 줄어든다(`.stu-sch{flex:0 1 auto}`).

**줄을 누르면 상세(`openStuModal`)가 열린다.** 그래서 `수정` 버튼을 뺐다 — 줄 탭과 같은 일을 한다. 남은 버튼(`학생뷰`·`퇴원`)은 감싼 `div` 에서 `event.stopPropagation()` 을 하므로 버튼을 눌러도 상세가 같이 열리지 않는다. 알림톡 선택 모드에서는 줄 탭이 `cardTapSelect` 라 상세가 열리지 않는다.
