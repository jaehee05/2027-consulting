...
**설명서가 아니라 앱처럼 쓴다.** `문제 1개에 1토큰이 차감됩니다` 가 아니라 `토큰 2개 사용 · 내 토큰 7개`, `스레드가 자동으로 종료됩니다` 가 아니라 `자동으로 닫혀요`. 학생 화면에서 **`스레드` 라는 말을 쓰지 않는다** — 그냥 `질문` 이다(관리자 화면도 같은 말로 맞췄다). 버튼은 동작을 그대로 적는다: `2토큰 쓰고 등록` → `토큰 2개로 질문하기`.
