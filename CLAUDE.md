...
- **한 스레드에는 문제 하나만.** 여러 문제를 한 번에 적으면 답변이 뒤섞여 학생도 관리자도 어디까지 답했는지 못 따라간다. **안내로만 걸고 서버에서 막지 않는다** — 글에서 문제 개수를 셀 방법이 없고, 잘못 막으면 정상 질문이 거부된다. 문구는 작성 화면·스레드 상세·[토큰·결제] 안내 세 군데에 같이 둔다(한 군데만 적으면 이미 스레드를 연 학생은 못 본다).
