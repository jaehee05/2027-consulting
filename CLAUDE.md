...
- **Input type comes from `akItemChoice(it)`**: if `a` is set, `1`–`5` means 객관식 and anything else 주관식; if `a` is **blank**, the per-item `s:1` flag decides (set from the 주관식 문항 field, e.g. `16-22,29-30`). That flag is what lets a key exist before the answers do.
- **A key can be saved with no answers at all** — the 정답 없이 열어두기 checkbox sets `open:true`, and `akSave` keeps such a subject so students can record their answers before 정답 발표. `swSheetStatus` then shows `정답이 등록되면 자동으로 채점됩니다` and leaves 원점수 blank.
