...
- 멘토링 shows a **정오표** via `wrongListHtml(exam, sc)` → `ojTablesHtml`: one section per subject that has both an answer key and saved answers, split 공통과목 / 선택과목(과목명) at `common`, then 10문항 per table with rows 번호 / 정답 / 내 답 / 채점 (O·X, 미입력은 `·`). It appears in both `renderConsultDetail` (컨설팅 탭) and `renderTabletConsult` (멘토링 태블릿) under the 성적표 card. `.oj-tbl` must keep `width:max-content` — the global `table{width:100%}` otherwise stretches the columns across the card.
