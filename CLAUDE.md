...
밑줄 스트립이던 것을 **세그먼트(알약 묶음)** 로 바꿨다. 한 영역에 두세 개뿐이라 `flex:1 1 0` 으로 폭을 나눠 가지면 어디에 있는지가 한눈에 보이고 손가락이 닿는 넓이도 같이 커진다. 관리자 화면 탭·학생 멘토링 속탭·질문게시판 속탭이 **모두 같은 클래스**를 쓴다. `.tabs-wrap` 은 topbar 아래에 `sticky` 라, 표를 길게 내려도 다른 화면으로 바로 건너뛸 수 있다.
