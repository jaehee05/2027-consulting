...
- **한 스레드에 문제는 2개까지.** 상한은 서버 `tokens.js` 의 `MAX_PROBLEMS` 와 화면의 `QA_MAX_PROBLEMS` **두 군데에 같은 값**으로 있다(질문 작성은 `tokenApi` 를 타므로 화면 상수만 고치면 서버가 잘라 버린다). 학생이 작성 화면에서 [문제 1개]/[문제 2개] 를 고르고, `createQuestion` 이 `normProblems` 로 1~상한 사이로 자른 뒤(없거나 이상한 값은 1) 그 배수로 잔액을 확인·차감하고 `problems`·`tokenCost` 를 문서에 남긴다. **개수를 글에서 세지는 않는다** — 셀 방법이 없고 잘못 막으면 정상 질문이 거부되므로, 3개 이상을 적는 것은 예전처럼 안내로만 말린다. 문구는 작성 화면·스레드 상세·[토큰·결제] 안내 세 군데에 같이 둔다(한 군데만 적으면 이미 스레드를 연 학생은 못 본다).
  ...
  - **댓글도 문제마다 따로 단다.** 댓글의 `pi`(0부터)가 어느 문제에 달린 것인지 말한다. 이 칸이 **없는** 댓글은 문제를 가리지 않은 것으로 본다 — 기능이 생기기 전에 달린 댓글과, `index.html` 을 번들로 들고 있어 이 칸 없이 보내는 스토어 배포본이 그렇다. 거부하지 않고(`problems` 가 없을 때 1개로 보는 것과 같은 이유) 문제 칸 **아래에 따로 모아** 보여 준다 — 양쪽 칸에 겹쳐 그리면 같은 글이 두 번 보인다. 범위를 벗어난 값도 같게 취급한다(`normProblemIndex`).
  - **문제마다 관리자 답이 있어야 답변완료다**(`allAnswered`). 하나만 답하고 답변완료로 넘어가면 남은 문제가 답변대기 필터에서 사라지는 데다, **자동 종료 시계까지 돌기 시작해 답을 못 받은 채 닫힌다**. 댓글을 지워 답이 빈 문제가 생기면 답변대기로 되돌린다 — `deleteQuestionComment` 도 같은 판정을 쓴다. 문제를 가리지 않은 관리자 댓글 하나는 둘 다 답한 것으로 본다(번들 앱이 보낸 답변이 영원히 답변대기로 남지 않게).
  - **화면은 나란히 보기와 확대 보기 둘이다.** 기본은 두 문제가 나란히(`.qa-pcols` / `.qa-pb`)이고 칸마다 그 문제의 댓글과 입력칸이 따로 붙는다. 칸의 **머리줄과 본문**을 누르면 그 문제만 폭을 다 쓰는 확대 보기로 들어간다(`qaTapProblem` → `S.qa.focusPi`). 사진은 누름 자리에서 뺀다 — 크게 보는 라이트박스가 이미 붙어 있다. 확대에서는 탭(`.tabs`)으로 문제를 오가고 [나란히 보기]로 돌아온다.
  ...
  - 토큰·새 질문·자동 종료 안내는 `qaThreadNotesHtml` 이 스레드 아래에 **한 번만** 둔다. 칸마다 되풀이하면 같은 말이 두 번 보인다.
